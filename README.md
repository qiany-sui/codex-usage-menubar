...
后续替换为新版本并重新启动 Codex Usage 时，应用会在主程序、助手、启动配置或安装路径发生变化后自动刷新后台注册，不需要手动删除旧登录项。仅更新主程序时也会刷新，避免旧注册失效导致无法跟随 Codex 启动。
...
“最近 7 天”的每日列表显示“当日额度消耗”：直接计算官方已用额度的当日增量，例如 38% → 45% 显示 7%，与 Token 数量无关。按本地自然日统计，今天统计到最近的额度快照。重置日显示重置时间，并分别展示“重置前 / 重置后”的消耗；一天多次重置时按时间顺序显示各段，悬停可查看每段的时间范围。每段百分比对应各自周期额度，不把跨重置合计显示成当前周期消耗，Token 列仍为全天总量。零点、日末和重置前使用不超过 10 分钟的记录；跨夜空档只有在同周期两侧读数相同时才能确认边界值。边界记录不足但有可确认增量时显示“已记录消耗”并标注“记录不完整”；重置日各段分别保留已记录增量，不合并成完整的全天百分比。没有可确认增量的分段显示“记录不足”，无重置日显示 `--`。数据精度和延迟取决于官方额度快照；“Token 状态”仅表示 Token 校准状态，缺失的额度记录不会凭空补成 0。

主界面“今日 Token”旁同步显示当日额度消耗；重置日沿用最近 7 天的分段口径，点击可查看每日详情。“当前周期 Token”和历史周期列表显示各自“额度已用”百分比：当前周期使用最新官方累计已用值，已结束周期仅采用该周期结束前 10 分钟内的最后一条官方记录，不把提前重置等同于额度耗尽。已结束周期缺少临近结束的记录时，保留该周期“最后记录”的已用百分比并标注“记录不完整”，悬停可查看采集时间；它不代表周期结束时的最终值。同周期读数下降、没有可用记录或只有估算边界时仍显示“额度记录不足”，不使用当前周期数值倒推。

历史周期结合官方额度记录识别重置：连续额度记录中已用额度仍为 0 时的起点修正归入同一次重置，后续刷新会自动修复已有碎片并重新汇总 Token。已发生额度消耗后的真实重置会保留，因此前后两个真实周期可能共享同一天作为边界日期。

右上角的调色盘菜单可切换“环形仪表”、“霓光仪表”和“极光卡片”。默认使用“极光卡片”，原“原生精修”的已存偏好会迁移到“极光卡片”。选择会自动保存，下次启动继续使用；深浅色跟随 macOS，趋势和历史页的强调色随样式切换。缓存 Token 已包含在输入 Token 中，明细标为“其中缓存”。

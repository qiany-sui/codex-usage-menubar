...
“最近 7 天”的每日列表显示“当日额度消耗”：直接计算官方已用额度的当日增量，例如 38% → 45% 显示 7%，与 Token 数量无关。按本地自然日统计，今天统计到最近的额度快照。重置日显示重置时间，并分别展示“重置前 / 重置后”的消耗；一天多次重置时按时间顺序显示各段，悬停可查看每段的时间范围。每段百分比对应各自周期额度，不把跨重置合计显示成当前周期消耗，Token 列仍为全天总量。零点、日末和重置前使用不超过 10 分钟的记录；跨夜空档只有在同周期两侧读数相同时才能确认边界值。某段记录不足时单独标注“记录不足”，保留其他可确认的分段；无重置日无法确定增量时显示 `--`。数据精度和延迟取决于官方额度快照；“Token 状态”仅表示 Token 校准状态，缺失的额度记录不会凭空补成 0。

主界面“今日 Token”旁同步显示当日额度消耗；重置日沿用最近 7 天的分段口径，点击可查看每日详情。“当前周期 Token”和历史周期列表显示各自“额度已用”百分比：当前周期使用最新官方累计已用值，已结束周期仅采用该周期结束前 10 分钟内的最后一条官方记录，不把提前重置等同于额度耗尽。历史记录不足、同周期读数下降导致结束值不明或只有估算边界时显示“额度记录不足”，不使用当前周期数值倒推。
